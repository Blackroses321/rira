...
contract ExclusiveCigarettesCrypto is ERC721AQueryable, Ownable, ReentrancyGuard {
    ...
    uint256 public ECCCost = 22 ether;  
    uint256 public ECCWLCost = 11 ether;
    uint256 public maxECCPerMint = 20;
    ...
    // Public functions
    function ECCMint(uint8 _mintAmount) external payable {
        uint256 nextTokenId = _nextTokenId();
        uint8 whiteListAmount = whiteList[msg.sender];

        require(!ECCMintPaused, "The ECCtible's minting is paused by the moment");
        ...
        require(_mintAmount <= maxECCPerMint, "Max ECC per mint reached"); 
        ...
        if(isWhiteListActive && (whiteListAmount > 0)){
            if(_mintAmount <= whiteListAmount){
                require(msg.value >= ECCWLCost * _mintAmount, "Insufficient funds"); 

                whiteList[msg.sender] -= _mintAmount;
                _safeMint(msg.sender, _mintAmount);
            }else if(isPublicSaleActive){
                require(msg.value >= (ECCWLCost * whiteListAmount) + (ECCCost * (_mintAmount - whiteListAmount)), "Insufficient funds"); 
            ...
            require(msg.value >= ECCCost * _mintAmount, "Insufficient funds");   
    ...
    // Owner functions
    function changeStateECCMintPause() external onlyOwner {
        ECCMintPaused = !ECCMintPaused;
    }

    function changeStateECCReveal() external onlyOwner {
        ECCRevealed = !ECCRevealed;
    }
    ...
    function setECCCost(uint256 _newECCCost) external onlyOwner {
        ECCCost = _newECCCost;
    }

    function setECCWLCost(uint256 _newECCWLCost) external onlyOwner {
        ECCWLCost = _newECCWLCost;
    }

    function setMaxECCPerMint(uint256 _newMaxECCPerMint) external onlyOwner {
        maxECCPerMint = _newMaxECCPerMint;
    }
