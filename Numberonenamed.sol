...
contract RIRAKKUSU is ERC721AQueryable, Ownable {
  using Strings for uint256;

  uint256 public constant TOTAL_MAX_SUPPLY = 10000;
  uint256 public totalFreeMints = 150;
  uint256 public teamAmount = 200;
  uint256 public maxFreeMintPerWallet = 2;
  uint256 public maxPublicMintPerWallet = 10;
  uint256 public publicTokenPrice = .08 ether;
  string _contractURI;
  ...
  constructor() ERC721A('RIRAKKUSU', 'RKU') {}

  modifier callerIsUser() {
    require(tx.origin == msg.sender, 'RIRAKKUSU: The caller is another contract');
    _;
  }
  ...
  modifier underMaxSupply(uint256 _quantity) {
    require(
      ...
      'RIRAKKUSU: Purchase exceeds max supply'
    );
    _;
  }

  function mint(uint256 _quantity) external payable callerIsUser underMaxSupply(_quantity) {
    require(balanceOf(msg.sender) < maxPublicMintPerWallet, "RIRAKKUSU: Mint limit exceeded");
    require(saleStarted, 'RIRAKKUSU: Sale has not begun ');
    if (_totalMinted() < (TOTAL_MAX_SUPPLY - teamAmount)) {
      if (freeMintCount >= totalFreeMints) {
        require(msg.value >= _quantity * publicTokenPrice, 'RIRAKKUSU: Send more ETH!');
        ...
        if (_quantity <= _mintableFreeQuantity) {
          ...
          require(
            ...
            'RIRAKKUSU:  Find more ETH to send'
          );
        }
        _mint(msg.sender, _quantity);
      } else {
        require(msg.value >= (_quantity * publicTokenPrice), 'RIRAKKUSU: Find more ETH to send');
        _mint(msg.sender, _quantity);
      }
    }
  }
  ...
  function withdrawFunds() external onlyOwner {
    ...
    require(success, 'RIRAKKUSU: Transfer failed.');
  }
